...
async fn do_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(response = interaction2.output_text(), "response 2 (uses server-side state)");
    ...
    info!(response = interaction3.output_text(), "response 3a (stateless)");
    ...
    info!(response = interaction4.output_text(), "response 4 (stateless, client-side history)");
